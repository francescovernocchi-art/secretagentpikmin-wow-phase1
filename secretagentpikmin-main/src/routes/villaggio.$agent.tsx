...
function PartnerBasePage() {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "base_buildings", filter: `agent=eq.${target}` },
        reload,
      )
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "bases", filter: `agent=eq.${target}` },
        reload,
      )
      .subscribe();
    return () => {
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          className="panel-strong p-3 flex flex-col items-center gap-1"
        >
      ...
      <div className="panel p-3 space-y-2">
        ...
        {buildings.length === 0 && (
          <p className="text-xs text-muted-foreground">Ancora nessuna struttura.</p>
        )}
        {buildings.map((b) => {
          const c = cat(b.type);
          if (!c) return null;
          return (
            <div
              key={b.id}
              className="flex items-center gap-3 rounded-lg bg-night/60 p-2 border border-primary/10"
            >
    ...
    >
      ...
      >
        ...
        <div className="flex gap-2">
          <button onClick={onClose} className="panel flex-1 py-2 text-xs">
            Annulla
          </button>
          <button
