...
function PrepareView({ templateKey }: { templateKey: string }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setAllBreakdown(
        ((data?.breakdown as Record<string, number>) ?? {}) as Record<string, number>,
      );
    })();
  }, [templateKey]);
  ...
  return (
    ...
    >
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-1.5 mb-1">
              <span
                className={`text-[10px] uppercase tracking-widest px-2 py-0.5 rounded-full border ${diff.color}`}
              >
                {diff.label}
              </span>
            ...
            <p className="text-[10px] text-muted-foreground mt-2">
              Min {template.pikmin_min} · Consigliati {template.pikmin_recommended} · Max{" "}
              {template.pikmin_max}
            </p>
          </div>
        </div>
      ...
      <div className="panel p-3 flex items-center gap-2 text-[11px] text-muted-foreground">
        ...
        <span>
          Spedizione in solo. Una volta lanciata potrai invitare{" "}
          {PARTNER_OF[agent] === "papa" ? "Papà" : "Lorenzo"} ad unirsi (bonus +15% successo).
        </span>
      </div>
      ...
      <div className="panel p-4 space-y-3">
        ...
        {allTypes.length === 0 && (
          <p className="text-xs text-muted-foreground">
            Non hai Pikmin disponibili. Esplora la mappa per reclutarne!
          </p>
        )}
        {allTypes.map((type) => {
          const max = allBreakdown[type] ?? 0;
          ...
          return (
            <div key={type}>
              <div className="flex items-center justify-between mb-1">
                <span
                  className={`text-xs uppercase tracking-widest ${isRec ? "text-primary text-glow" : "text-muted-foreground"}`}
                >
      ...
      {preview && total > 0 && (
        <div className="panel-strong p-4 space-y-3 relative overflow-hidden">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            ...
            <Cell
              label="Rischio"
              value={RISK_META[preview.risk].label}
              accent={RISK_META[preview.risk].color}
            />
...
function ExpeditionView({ id }: { id: string }) {
  ...
  const pct =
    exp.status === "active" ? Math.max(0, Math.min(100, 100 - (leftMs / totalMs) * 100)) : 100;
  ...
  return (
    ...
    >
      <div
        className={`panel-strong p-4 bg-gradient-to-br ${biome?.color ?? ""} relative overflow-hidden`}
      >
        <span className="hud-corner tl" />
        <span className="hud-corner br" />
        ...
        {exp.status === "waiting_partner" && (
          <p className="text-sm text-amber-200">
            ⏳ In attesa che {PARTNER_OF[exp.created_by] === "papa" ? "Papà" : "Lorenzo"} confermi
            la sua squadra…
          </p>
        )}
        {(exp.status === "completed" || exp.status === "failed") && (
          <p className="text-sm">
            ...
            {exp.result === "parziale" && (
              <span className="text-yellow-300">~ Risultato parziale</span>
            )}
            {exp.result === "fallito" && (
              <span className="text-rose-300">✗ Spedizione fallita</span>
            )}
          </p>
        )}
      ...
      {canInvitePartner && (
        <InvitePartnerButton expeditionId={exp.id} agent={agent} onInvited={load} />
      )}

      {/* IN ATTESA RISPOSTA PARTNER */}
      {exp.created_by === agent &&
        exp.is_coop &&
        !hasOwnSquadOther(squads, exp.partner) &&
        exp.status === "active" && (
          <div className="panel p-3 text-[11px] text-amber-200 flex items-center gap-2">
            <Sparkles className="h-3 w-3" />
            Invito inviato a {exp.partner === "papa" ? "Papà" : "Lorenzo"} · in attesa che si
            unisca…
          </div>
        )}
      ...
      {(exp.status === "completed" || exp.status === "failed") && exp.rewards && (
        <div className="panel-strong p-4 space-y-2">
          ...
          {(exp.rewards as any).ingredients?.length > 0 && (
            <p className="text-sm">
              🧪 Ingredienti: {((exp.rewards as any).ingredients as string[]).join(", ")}
            </p>
          )}
