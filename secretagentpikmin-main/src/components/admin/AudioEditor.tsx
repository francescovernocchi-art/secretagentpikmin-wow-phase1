...
export function AudioEditor() {
  ...
  const save = async (a: Asset) => {
    ...
    else toast.success("Audio salvato");
  };
    ...
    <div className="flex flex-col gap-3">
      <label className="panel-strong p-3 text-xs cursor-pointer text-center">
        ...
        <input
          ...
          accept="audio/*"
          className="hidden"
          onChange={(e) => e.target.files?.[0] && onNew(e.target.files[0])}
        />
      </label>
      {list.map((a) => (
        <div key={a.id} className="panel-strong p-3 flex flex-col gap-2 text-xs">
          <div className="flex items-center gap-2">
            ...
            />
            <button onClick={() => del(a.id)} className="panel p-1.5 text-rose-400">
          ...
          <div className="grid grid-cols-3 gap-1">
            ...
            <select
              className="input"
              value={a.page ?? "global"}
              onChange={(e) => update(a.id, { page: e.target.value })}
            >
              {PAGES.map((p) => (
            ...
            <label className="flex items-center gap-1">
              <input
                type="checkbox"
                checked={a.loop}
                onChange={(e) => update(a.id, { loop: e.target.checked })}
              />{" "}
              Loop
            </label>
            <label className="col-span-2">
              ...
              <input
                ...
                step={0.05}
                value={a.volume}
                onChange={(e) => update(a.id, { volume: Number(e.target.value) })}
                className="w-full"
              />
            </label>
            <label className="flex items-center gap-1">
