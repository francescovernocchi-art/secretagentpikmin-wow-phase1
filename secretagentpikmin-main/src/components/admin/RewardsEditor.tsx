...
export function RewardsEditor() {
  ...
  const update = (id: string, patch: Partial<Part>) =>
    setParts((p) => p.map((c) => (c.id === id ? { ...c, ...patch } : c)));
  ...
  const save = async (p: Part) => {
    ...
    else toast.success("Pezzo salvato");
  };
  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="text-[11px] text-muted-foreground">
        Pezzi della navicella & premi. Per le medaglie usa la tabella <code>rewards</code> dal
        pannello premi del gioco.
      </p>
        ...
        {parts.map((p) => (
          <div key={p.id} className="panel-strong p-2 flex flex-col gap-2 text-xs">
            ...
            <select
              className="input"
              value={p.rarity}
              onChange={(e) => update(p.id, { rarity: e.target.value })}
            >
            ...
            <div className="flex gap-1">
              <button
                onClick={() => save(p)}
                disabled={busy === p.id}
                className="flex-1 panel-strong p-1.5 flex items-center justify-center gap-1"
              >
                <Save className="h-3 w-3" />
                Salva
              </button>
              <button onClick={() => del(p.id)} className="panel p-1.5 text-rose-400">
