...
export function CardsEditor() {
  ...
  const update = (id: string, patch: Partial<Card>) =>
    setList((p) => p.map((c) => (c.id === id ? { ...c, ...patch } : c)));
  ...
  const save = async (c: Card) => {
    ...
    else toast.success("Carta salvata");
  };
          ...
          <div key={c.id} className="panel-strong p-2 flex flex-col gap-2">
            ...
            <label className="text-[10px] panel px-2 py-1 cursor-pointer text-center">
              ...
              <input
                ...
                accept="image/*"
                className="hidden"
                onChange={(e) => e.target.files?.[0] && onUpload(c, e.target.files[0])}
              />
            </label>
            <input
            ...
            <div className="grid grid-cols-2 gap-1">
              <select
                className="input text-xs"
                value={c.rarity}
                onChange={(e) => update(c.id, { rarity: e.target.value })}
              >
            ...
            <div className="flex gap-1">
              <button
                onClick={() => save(c)}
                disabled={busy === c.id}
                className="flex-1 panel-strong p-1.5 text-xs flex items-center justify-center gap-1"
              >
                <Save className="h-3 w-3" /> Salva
              </button>
              <button onClick={() => del(c.id)} className="panel p-1.5 text-xs text-rose-400">
