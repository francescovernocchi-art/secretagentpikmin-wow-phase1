  ...
  return (
    <section className="space-y-3">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Ogni Pikmin cresce in base alle missioni svolte: raccolta, ricerca, scouting, spionaggio,
          trasporto o combattimento.
        </p>
      </div>
        ...
        {PIKMIN_SPECIALIZATIONS.map((s) => {
          ...
          return (
            <div key={s.key} className="panel p-3 space-y-2">
              <div className="flex items-center gap-2">
                <div className="h-9 w-9 rounded-xl bg-primary/10 border border-primary/25 grid place-items-center text-lg">
                  {s.emoji}
                </div>
                <div>
                  <p className="font-display text-sm text-glow">{s.title}</p>
                  <p className="text-[10px] text-primary/80 flex items-center gap-1">
                    <Icon className="h-3 w-3" /> tipi consigliati: {s.bestTypes.join(", ")}
                  </p>
                </div>
              </div>
              <div className="flex flex-wrap gap-1">
                {s.duties.map((d) => (
                  <span
                    key={d}
                    className="rounded-full bg-night/60 border border-border px-2 py-0.5 text-[10px] text-muted-foreground"
                  >
                    {d}
                  </span>
                ))}
              ...
              <div className="flex flex-wrap gap-1">
                {s.bonuses.map((b) => (
                  <span
                    key={b}
                    className="rounded-full bg-primary/10 border border-primary/20 px-2 py-0.5 text-[10px] text-primary"
                  >
                    + {b}
                  </span>
                ))}
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="space-y-2">
        <p className="text-[10px] uppercase tracking-[0.35em] text-primary/80">
          // biomi geolocalizzati
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {GEO_BIOME_RULES.map((b) => (
            <div key={b.key} className="panel p-3">
              <div className="flex items-center justify-between gap-2">
                <p className="font-display text-sm text-glow">
                  <span className="mr-1">{b.emoji}</span>
                  {b.label}
                </p>
                <span className="rounded-full bg-primary/10 px-2 py-0.5 text-[9px] text-primary uppercase tracking-wider">
                  {b.bonus}
                </span>
              </div>
              <p className="mt-2 text-[10px] text-muted-foreground">
                Materiali: {b.materials.join(", ")}
              </p>
              <p className="text-[10px] text-muted-foreground">
                Pikmin frequenti: {b.pikmin.join(", ")} · Minacce: {b.threats.join(", ")}
              </p>
            </div>
          ))}
        </div>
