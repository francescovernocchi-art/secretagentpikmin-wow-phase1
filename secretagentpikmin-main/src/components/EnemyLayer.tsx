...
export function EnemyLayer({ mapRef, ready, me }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const live = (data ?? []).filter(
        (s: Spawn) => !s.expires_at || new Date(s.expires_at).getTime() > now,
      );
    ...
    const ch = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "map_enemy_spawns" }, () =>
        load(),
      )
      .subscribe();
    return () => {
  ...
  useEffect(() => {
    const tick = () => {
      ...
      for (const s of spawns) {
        ...
        const fromHome = distMeters(
          { lat: nextLat, lng: nextLng },
          { lat: pos.homeLat, lng: pos.homeLng },
        );
        ...
        if (fromHome > maxRadius) {
          ...
          const back2 =
            (stepM * Math.sin(heading)) / (111320 * Math.cos((pos.lat * Math.PI) / 180));
        ...
        for (const s of spawns) {
          ...
          if (d <= pos.detectionM && Date.now() > dismissed) {
            if (!detectedRef.current.has(s.id)) {
              ...
              toast.error(`⚠️ NEMICO VICINO · ${enemy.name} (${Math.round(d)}m)`, {
                duration: 4000,
              });
            }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const s of spawns) {
        ...
        const nearby = me
          ? distMeters(pos, me) <= (livePosRef.current.get(s.id)?.detectionM ?? DETECTION_RADIUS_M)
          : false;
        ...
        if (existing) {
          ...
          if (el) {
            ...
            if (ico)
              (ico as HTMLElement).style.filter =
                `drop-shadow(0 0 8px ${color})${sleeping ? " grayscale(0.4)" : ""}`;
          }
        } else {
  ...
  const fight = async () => {
    ...
    if (res.outcome === "vittoria" && res.rewards.coins > 0) {
      try {
        await addCoins(agent, res.rewards.coins, "battle_reward", { enemy: enemy.name });
      } catch {
        /* ignore */
      }
    }
        ...
        <>
          ...
          {placeEnemyMode && (
            <div className="fixed top-44 right-3 z-[1100] panel px-3 py-1.5 text-[10px] uppercase tracking-widest text-destructive border-destructive/40">
              ...
              <button
                onClick={() => setPlaceEnemyMode(null)}
      ...
      <Dialog open={placeEnemyOpen} onOpenChange={setPlaceEnemyOpen}>
        <DialogContent className="max-w-sm max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Poi tocca la mappa nel punto in cui vuoi farlo apparire.
            </DialogDescription>
          </DialogHeader>
          ...
          {card &&
            (() => {
              ...
              const dist = me
                ? Math.round(
                    distMeters(
                      { lat: live?.lat ?? card.spawn.lat, lng: live?.lng ?? card.spawn.lng },
                      me,
                    ),
                  )
                : null;
              return (
                <>
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      {card.enemy.habitat ?? "Avvistato sulla mappa"}
                      {dist !== null ? ` · ${dist}m da te` : ""}
                    </DialogDescription>
                  </DialogHeader>
                  <WikiImage
                    src={card.enemy.image_url}
                    alt={card.enemy.name}
                    fallback={card.enemy.emoji}
                  ...
                  {isScanned ? (
                    <div className="panel p-2 space-y-1 text-[11px]">
                      <p className="flex items-center gap-1 text-primary">
                        <RadarIcon className="h-3 w-3" /> SCAN COMPLETATO
                      </p>
                      <p>
                        Pericolosità:{" "}
                        <b className="text-destructive">{card.enemy.danger_level}/5</b> · HP{" "}
                        {card.enemy.hp} · Danno {card.enemy.damage}
                      </p>
                      <p>Comportamento: {live?.behavior ?? card.enemy.behavior ?? "?"}</p>
                      <p>
                        Pikmin consigliati:{" "}
                        {(card.enemy.recommended_pikmin ?? []).length === 0
                          ? "—"
                          : (card.enemy.recommended_pikmin ?? []).map((t) => (
                              <span key={t} className="mr-1">
                                {PIKMIN_TYPE_EMOJI[t as PikminType]}{" "}
                                {PIKMIN_TYPE_LABEL[t as PikminType]}
                              </span>
                            ))}
                      </p>
                    </div>
                    ...
                    <p className="text-[11px] text-muted-foreground italic">
                      Dati sconosciuti. Esegui uno scan per rivelare pericolosità e Pikmin
                      consigliati.
                    </p>
                  )}
                  <div className="grid grid-cols-3 gap-2 mt-3">
                    <button
                      onClick={() => {
                        ...
                        toast.success(
                          `Scan: ${card.enemy.name} pericolosità ${card.enemy.danger_level}/5`,
                        );
                      }}
                    ...
                    <button
                      onClick={() => {
                        setHiddenUntil(Date.now() + HIDE_DURATION_MS);
                        proximityDismissedRef.current.set(
                          card.spawn.id,
                          Date.now() + HIDE_DURATION_MS,
                        );
                        try {
                          navigator.vibrate?.(20);
                        } catch {
                        ...
                        toast.message(
                          `Sei nascosto per ${HIDE_DURATION_MS / 1000}s. ${card.enemy.name} non ti vede.`,
                        );
                        setCard(null);
                      }}
                      className="panel py-2 text-xs flex items-center justify-center gap-1"
          ...
          {proximity && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  {proximity.enemy.emoji} <b>{proximity.enemy.name}</b> ·{" "}
                  {Math.round(proximity.dist)}m · pericolosità {proximity.enemy.danger_level}/5
                </DialogDescription>
              </DialogHeader>
              <div className="text-[11px] text-muted-foreground -mt-1">
                HP {proximity.enemy.hp} · Danno {proximity.enemy.damage} · Comp.:{" "}
                {livePosRef.current.get(proximity.spawn.id)?.behavior ?? "?"}
              </div>
              <div className="grid grid-cols-2 gap-2 mt-3">
                <button
                  onClick={openBattleFromProximity}
                  className="btn-neon py-2 text-xs flex items-center justify-center gap-1"
                >
          ...
          {active && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  Pericolosità {active.enemy.danger_level}/5 · HP {active.enemy.hp} · Danno{" "}
                  {active.enemy.damage}
                </DialogDescription>
              </DialogHeader>

              <WikiImage
                src={active.enemy.image_url}
                alt={active.enemy.name}
                fallback={active.enemy.emoji}
                className="w-full h-36 p-3"
              />
                      ...
                      <div key={t} className="panel p-2">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setSquad({
                              ...squad,
                              [t]: Math.max(0, Math.min(max, Number(e.target.value))),
                            })
                          }
              ...
              <div className="flex gap-2 pt-3">
                <button
                  onClick={closeBattle}
                  className="flex-1 panel py-2 text-xs flex items-center justify-center gap-1"
                >
