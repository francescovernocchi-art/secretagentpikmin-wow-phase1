...
export function AestheticsPanel({
  ...
  const update = (p: PikminLayerPrefs) => {
    onPrefsChange(p);
    savePikminPrefs(p);
  };
    ...
    <>
      ...
      >
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {species.length === 0 && (
                <p className="text-[11px] text-muted-foreground italic">
                  Aggiungi specie dalla Libreria Sprite (admin).
                </p>
              )}
              {species.map((s) => {
                ...
                return (
                  <button
                    key={s.key}
                    onClick={() =>
                      update({ ...prefs, filters: { ...prefs.filters, [s.key]: !on } })
                    }
                    className={`px-2.5 py-1 rounded-full border text-[10px] transition ${
                      on
                        ? "bg-primary/20 border-primary/50"
                        : "border-muted-foreground/30 text-muted-foreground opacity-60"
                    }`}
                  >
                    <span
          ...
          {isPapa && (
            <button
              onClick={() => setBiomeAdminOpen(true)}
              className="btn-neon w-full py-2 text-xs inline-flex items-center justify-center gap-2"
            >
          ...
          {isPapa && (
            <button
              onClick={() => setEventsAdminOpen(true)}
              className="btn-neon w-full py-2 text-xs inline-flex items-center justify-center gap-2"
            >
          ...
          <button onClick={() => setCustomizerOpen(true)} className="btn-neon w-full py-2 text-xs">
