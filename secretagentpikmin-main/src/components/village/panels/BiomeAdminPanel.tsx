...
export function BiomeAdminPanel({ open, onOpenChange, onChanged }: Props) {
  ...
  const save = async () => {
    ...
    const key = draft.key
      ...
      .replace(/[^a-z0-9_]/g, "");
    if (!key) {
      toast.error("Chiave obbligatoria (es. cyberpunk)");
      return;
    }
    if (!draft.label.trim()) {
              ...
              {rows.map((r) => (
                <div key={r.id} className="panel p-2 flex items-center gap-2">
                  {r.image_url ? (
                    ...
                    <div
                      className="w-12 h-12 rounded grid place-items-center text-xl"
                      style={{ background: r.accent + "33" }}
                    >
                      {r.emoji}
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground truncate">
                      {r.key} · {r.is_active ? "attivo" : "nascosto"}
                    </p>
                  </div>
                  <button
                    onClick={() => startEdit(r)}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1 mb-1">
                {draft.bonuses.map((b, i) => (
                  <span
                    key={i}
                    className="text-[10px] px-2 py-0.5 rounded-full bg-primary/20 inline-flex items-center gap-1"
                  >
                    {b}
                    <button onClick={() => removeBonus(i)} className="opacity-70 hover:opacity-100">
                      ×
                    </button>
                  </span>
                ))}
              ...
              <div className="flex gap-1">
                ...
                <button
                  onClick={addBonus}
                  className="px-2 rounded bg-primary/20 hover:bg-primary/30 text-[11px]"
                >
                  +
                </button>
              </div>
