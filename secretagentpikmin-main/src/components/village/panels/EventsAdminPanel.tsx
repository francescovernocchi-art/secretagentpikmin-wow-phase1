...
export function EventsAdminPanel({ open, onOpenChange, onChanged }: Props) {
  ...
  const applyPreset = (t: VillageEventType) => {
    ...
    setEditing({
      ...editing,
      event_type: t,
      icon: EVENT_TYPE_ICON[t],
      effects: EVENT_TYPE_PRESET[t],
    });
  };
  ...
  const toggleActive = async (ev: VillageEventRow) => {
    const ends =
      !ev.is_active && ev.duration_minutes
        ? new Date(Date.now() + ev.duration_minutes * 60_000).toISOString()
        : null;
    await supabase
      .from("village_diorama_events")
      .update({
        is_active: !ev.is_active,
        starts_at: !ev.is_active ? new Date().toISOString() : null,
        ends_at: ends,
      })
      .eq("id", ev.id);
    reload();
  ...
  const addBonus = () => {
    ...
    setEditing({
      ...editing,
      bonuses: [...(editing.bonuses ?? []), { key: "energy_max", amount: 0 }],
    });
  };
          ...
          {events.map((ev) => (
            <div key={ev.id} className="panel p-3 space-y-1.5">
              ...
              <div className="flex gap-1.5">
                <button onClick={() => setEditing(ev)} className="btn-neon flex-1 py-1 text-[10px]">
                  Modifica
                </button>
                <button
                  onClick={() => remove(ev)}
                  className="px-2 py-1 rounded border border-rose-500/40 text-rose-400"
                >
        ...
        <div className="space-y-3 text-[11px]">
          ...
          <div className="grid grid-cols-2 gap-2">
            <label className="space-y-1">
              ...
              <select
                value={editing.event_type ?? "custom"}
          ...
          <div className="space-y-1">
            ...
            {editing.overlay_image_url && (
              <img
                src={editing.overlay_image_url}
                alt=""
                className="w-full h-20 object-cover rounded border border-primary/20"
              />
            )}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            {(editing.bonuses ?? []).map((b, i) => (
              <div key={i} className="flex gap-1.5 items-center">
                <select
                  value={b.key}
                  onChange={(e) => setBonus(i, { key: e.target.value })}
            ...
            <p className="text-[9px] text-muted-foreground italic">
              Valori positivi = bonus, negativi = malus.
            </p>
          </div>

          <div className="flex gap-2 pt-2">
            <button onClick={() => setEditing(null)} className="flex-1 py-2 text-xs panel">
              Annulla
            </button>
            <button
              disabled={busy || !editing.key || !editing.name}
              onClick={save}
              className="flex-1 btn-neon py-2 text-xs disabled:opacity-50"
            >
              {busy ? "Salvo…" : "Salva"}
            </button>
          </div>
