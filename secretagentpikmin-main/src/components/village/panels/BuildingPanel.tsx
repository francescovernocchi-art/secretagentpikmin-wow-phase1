...
import { Building2, ArrowUpRight, Clock, Sparkles, Zap, Trash2 } from "lucide-react";
...
export function BuildingPanel({
  ...
  const nextImg =
    building && c ? pickBuildingImage(set, Math.min(c.max_level, building.level + 1)) : null;
  ...
  const total =
    building?.build_end_at && building.started_at
      ? Math.max(
          1,
          new Date(building.build_end_at).getTime() - new Date(building.started_at).getTime(),
        )
      : 1;
    ...
    >
      {!building || !c ? (
        <p className="text-xs text-muted-foreground text-center py-6">
          Seleziona un edificio sul diorama.
        </p>
      ) : (
        <div className="space-y-3">
                ...
                <div className="flex items-center justify-between text-[10px]">
                  <span className="text-amber-300 flex items-center gap-1">
                    ...
                    {building.status === "upgrading"
                      ? `Upgrade a Lv ${building.level + 1}`
                      : "Costruzione in corso"}
                  </span>
                  <span className="font-mono text-amber-400">
                    {formatRemaining(building.build_end_at)}
                  </span>
                </div>
          ...
          {/* DESCRIZIONE */}
          {c.description && (
            <p className="text-[11px] text-muted-foreground leading-snug px-1">{c.description}</p>
          )}
          ...
          {!isMax && !isBusy && nextCost && (
            <div className="panel-strong p-3 space-y-2">
              ...
              <div className="flex items-center gap-3 text-[11px]">
                ...
                <span className="flex items-center gap-1">
                  <Clock className="h-3 w-3" /> {nextCost.minutes}m
                </span>
          ...
          {isMax && (
            <div className="panel p-3 text-center">
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5">
                Questa struttura è al suo picco evolutivo.
              </p>
            </div>
          )}
