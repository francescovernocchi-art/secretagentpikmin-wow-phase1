      ...
      {!compact && (
        <header>
          ...
          <p className="text-xs text-muted-foreground">
            {villages.length}/{maxV} villaggi · CC regola il limite
          </p>
        </header>
      )}
          ...
          >
            <div className="flex justify-between items-center">
              ...
              <span className="text-primary">
                {BIOMES.find((b) => b.key === v.biome_key)?.emoji}
              </span>
            </div>
            ...
            {active?.id === v.id && (
              <p className="text-[10px] text-primary mt-1">● Villaggio attivo</p>
            )}
          </button>
        ))}
      </div>
        ...
        (showForm ? (
          <div className="panel p-3 space-y-2 text-xs">
            <input
              ...
              placeholder="Nome villaggio"
              className="w-full rounded-lg bg-night/60 border border-border px-2 py-1.5"
            />
            <select
              value={biome}
            ...
            <div className="flex gap-2">
              <button
                onClick={create}
                disabled={busy}
                className="btn-neon flex-1 py-2 flex items-center justify-center gap-1"
              >
                {busy ? (
              ...
              <button onClick={() => setShowForm(false)} className="panel flex-1 py-2">
                Annulla
              </button>
            </div>
          </div>
