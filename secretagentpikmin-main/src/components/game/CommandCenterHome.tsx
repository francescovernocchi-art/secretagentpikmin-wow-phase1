...
import { Bell, Map, MessageSquare, Radio, Rocket, ScanLine, Target, Users } from "lucide-react";
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="relative p-4 pb-0">
          <div className="flex items-start justify-between gap-3 mb-3">
            <div>
              <p className="text-[10px] uppercase tracking-[0.4em] text-primary/80 font-display">
                // Plancia di Comando
              </p>
      ...
      >
        ...
        <div className="flex justify-center -mt-2">
          <Link
            to="/villaggio"
            onClick={hapticTap}
            className="btn-neon px-4 py-1.5 text-[10px] uppercase tracking-widest"
          >
            Entra nel Villaggio →
          </Link>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        >
          ...
          <span className="hud-corner tl" />
          <p className="text-[10px] uppercase tracking-widest text-[var(--ad-nature)] mb-1">
            Pianeta in pericolo
          </p>
          <p className="text-xs text-muted-foreground mb-2">
            Ogni credito conta per salvare il mondo nativo.
          </p>
          ...
          <div className="mt-2 progress-mission">
            <div
              className="progress-mission-fill progress-planet-fill"
              style={{ width: `${debtBar}%` }}
            />
          </div>
          ...
          <div className="flex gap-1 mb-3 flex-wrap">
            {(data?.shipParts ?? []).map((p) => (
              <span
                key={p.key}
                title={p.name}
                className={`text-lg ${p.collected ? "opacity-100 drop-shadow-[0_0_6px_var(--color-primary)]" : "opacity-25 grayscale"}`}
              >
                {p.emoji}
              </span>
            ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <GamePanel
          ...
          count={(data?.expeditions.length ?? 0) + (data?.activeMissionCount ?? 0)}
        >
          <ul className="space-y-2">
            ...
            {!data?.expeditions.length && !loading && (
              <li className="text-xs text-muted-foreground">Nessuna spedizione attiva</li>
            )}
          </ul>
          <Link
            to="/spedizioni"
            onClick={hapticTap}
            className="text-[10px] text-primary uppercase tracking-widest mt-2 inline-block"
          >
            Tutte →
          </Link>
        </GamePanel>
        ...
        <GamePanel title="Ultimi ritrovamenti" icon={<Radio className="h-3 w-3" />}>
          <ul className="space-y-2">
            ...
            {!data?.discoveries.length && !loading && (
              <li className="text-xs text-muted-foreground">Scansiona un'area</li>
            )}
          </ul>
        </GamePanel>
      </div>
    ...
    <section className="panel-strong p-4 space-y-2 relative overflow-hidden">
      <p className="text-[10px] uppercase tracking-widest text-primary/80 flex items-center gap-1">
        ...
        {count !== undefined && count > 0 && (
          <span className="ml-auto panel px-1.5 py-0.5 text-[9px] text-primary">{count}</span>
        )}
      </p>
      {children}
    </section>
...
function QuickLink({ to, icon, label }: { to: string; icon: React.ReactNode; label: string }) {
  return (
    <Link
      to={to}
      onClick={hapticTap}
      className="panel-strong p-3 flex flex-col items-center gap-1.5 active:scale-95 transition hover:ring-1 hover:ring-primary/35"
    >
      {icon}
