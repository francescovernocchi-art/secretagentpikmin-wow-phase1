...
const FOOD_KEYS =
  /frutt|miele|seme|bacc|fungo|erba|carota|patat|insal|verdur|ingrediente|mela|pera|uva|noci/i;
const ENERGY_KEYS = /cristall|batter|energia|condens|plasma|carbur|nucleo|fulmine|elettr/i;
const MATERIAL_KEYS =
  /rottam|metall|ferro|rame|legno|pietra|mineral|vite|bullone|schegg|componente|materiale/i;
...
export async function transformInventory(
  ...
  switch (target) {
    case "food": {
      ...
      return {
        success: true,
        message: `+${gain}% cibo planetario da ${totalQty} ingredienti`,
        planetDelta: { food: gain },
      };
    }
    case "energy": {
      ...
      return {
        success: true,
        message: `+${gain}% energia da ${totalQty} componenti`,
        planetDelta: { energy: gain },
      };
    }
    case "materials": {
      await addInventoryItemInternal(
        agentKey,
        "materiali_raffinati",
        "Materiali raffinati",
        "🔩",
...
export const TRANSFORM_LABELS: Record<
  TransformTarget,
  { label: string; emoji: string; desc: string }
> = {
  food: {
    label: "Trasforma in Cibo",
    emoji: "🍎",
    desc: "Frutta e ingredienti → rifornimenti pianeta",
  },
  energy: { label: "Trasforma in Energia", emoji: "⚡", desc: "Cristalli e batterie → energia" },
  materials: {
    label: "Trasforma in Materiali",
    emoji: "🔩",
    desc: "Rottami e metallo → materiali da costruzione",
  },
  credits: {
    label: "Vendi per Crediti",
    emoji: "💰",
    desc: "Reliquie e oggetti rari → crediti al debito",
  },
};
