...
export function canAttackEnemy(player: GeoPoint | null, enemy: GeoPoint): boolean {
  ...
  return (
    calculateDistanceMeters(player.lat, player.lng, enemy.lat, enemy.lng) <= PLAYER_ATTACK_RADIUS
  );
}
...
export function canScoutEnemy(player: GeoPoint | null, enemy: GeoPoint): boolean {
  if (!player) return false;
  return (
    calculateDistanceMeters(player.lat, player.lng, enemy.lat, enemy.lng) > SCOUTING_MIN_DISTANCE
  );
}
